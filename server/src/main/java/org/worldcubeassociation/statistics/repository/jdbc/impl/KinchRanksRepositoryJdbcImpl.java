...
@Repository
@AllArgsConstructor
public class KinchRanksRepositoryJdbcImpl implements KinchRanksRepositoryJdbc {
    private final JdbcTemplate jdbcTemplate;
    private final NamedParameterJdbcTemplate namedJdbcTemplate;
    private final ObjectMapper objectMapper;

    private int baseRank(String singleQuery, String avgQuery) {
        int single = jdbcTemplate.update(StatisticsUtil.getQuery(String.format("kinchranks/%s", singleQuery)));
        int avg = jdbcTemplate.update(StatisticsUtil.getQuery(String.format("kinchranks/%s", avgQuery)));
        return single + avg;
    }
    ...
    public List<SumOfRanksMetaDto> getResultTypes() {
        return jdbcTemplate
                .query(StatisticsUtil.getQuery("kinchranks/getMeta"), JdbcTemplateMapperFactory.newInstance()
    ...
    public int insertMeta() {
        return jdbcTemplate.update(StatisticsUtil.getQuery("kinchranks/insertMeta"));
    }

    @Override
    public void updateRanks() {
        jdbcTemplate.update(StatisticsUtil.getQuery("kinchranks/updateRank"));
    }

    @Override
    public Optional<Integer> getWcaIdPage(String resultType, String regionType, String region, int pageSize,
                                          String wcaId) {
        List<Integer> list = namedJdbcTemplate.queryForList(StatisticsUtil.getQuery("kinchranks/getWcaIdPage"),
